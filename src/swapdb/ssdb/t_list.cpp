...
int SSDBImpl::GetListItemValInternal(const std::string &item_key, std::string *val, const rocksdb::ReadOptions &options) {
    rocksdb::Status s = ldb->Get(options, item_key, val);
    if (s.IsNotFound()){
        return 0;
...
int SSDBImpl::doListPush(Context &ctx, const Bytes &key, rocksdb::WriteBatch &batch, const std::vector<T> &val, int offset, std::string &meta_key,
    ...
    for (int i = offset; i < size; ++i) {
        ...
        batch.Put(item_key, rocksdb::Slice(val[i].data(), val[i].size()));
    }
...
//        log_fatal("invalid list index.");
    ...
    batch.Put(item_key, rocksdb::Slice(val.data(),val.size()));
