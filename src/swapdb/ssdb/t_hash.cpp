...
HIterator* SSDBImpl::hscan_internal(Context &ctx, const Bytes &name, uint16_t version, const rocksdb::Snapshot *snapshot){
...
int SSDBImpl::GetHashItemValInternal(const std::string &item_key, std::string *val){
	rocksdb::Status s = ldb->Get(rocksdb::ReadOptions(), item_key, val);
	if (s.IsNotFound()){
		return 0;
	} else if (!s.ok() && !s.IsNotFound()){
...
int SSDBImpl::incr_hsize(Context &ctx, const Bytes &name, rocksdb::WriteBatch &batch, const std::string &size_key, HashMetaVal &hv, int64_t incr) {
	int ret = 1;
...
int SSDBImpl::hset_one(rocksdb::WriteBatch &batch, const HashMetaVal &hv, bool check_exists, const Bytes &name,
