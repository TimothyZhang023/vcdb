...
public:
	...
	virtual rocksdb::Status CommitBatch(Context &ctx, const rocksdb::WriteOptions& options, rocksdb::WriteBatch* updates);
...
private:
	...
	int SetGeneric(Context &ctx, const Bytes &key, rocksdb::WriteBatch &batch, const Bytes &val, int flags, int64_t expire_ms, int *added);
    ...
    int mark_key_deleted(Context &ctx, const Bytes &key, rocksdb::WriteBatch &batch, const std::string &meta_key, std::string &meta_val);
    ...
    HIterator* hscan_internal(Context &ctx, const Bytes &name, uint16_t version, const rocksdb::Snapshot *snapshot=nullptr);
    int incr_hsize(Context &ctx, const Bytes &name, rocksdb::WriteBatch &batch, const std::string &size_key, HashMetaVal &hv, int64_t incr);
    int hset_one(rocksdb::WriteBatch &batch, const HashMetaVal &hv, bool check_exists, const Bytes &name, const Bytes &key, const Bytes &val);
    int GetSetMetaVal(const std::string &meta_key, SetMetaVal &sv);
    int GetSetItemValInternal(const std::string &item_key);
    SIterator* sscan_internal(Context &ctx, const Bytes &name, uint16_t version, const rocksdb::Snapshot *snapshot=nullptr);
    int incr_ssize(Context &ctx, const Bytes &key, rocksdb::WriteBatch &batch, const SetMetaVal &sv, const std::string &meta_key,int64_t incr);

	int GetListItemValInternal(const std::string &item_key, std::string *val, const rocksdb::ReadOptions &options = rocksdb::ReadOptions());
    int GetListMetaVal(const std::string &meta_key, ListMetaVal &lv);
    int doListPop(Context &ctx, const Bytes &key, rocksdb::WriteBatch &batch, ListMetaVal &lv, std::string &meta_key, LIST_POSITION lp, std::pair<std::string, bool> &val);

	template <typename T>
	int doListPush(Context &ctx, const Bytes &key, rocksdb::WriteBatch &batch, const std::vector<T> &val, int offset, std::string &meta_key, ListMetaVal &meta_val, LIST_POSITION lp);
    ...
    ZIteratorByLex* zscanbylex_internal(Context &ctx, const Bytes &name,const Bytes &key_start, const Bytes &key_end,
							  uint64_t limit, Iterator::Direction direction, uint16_t version,
							  const rocksdb::Snapshot *snapshot=nullptr);
	int	zset_one(rocksdb::WriteBatch &batch, bool needCheck, const Bytes &name, const Bytes &key, double score, uint16_t cur_version, int *flags, double *newscore);
