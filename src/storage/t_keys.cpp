...
//                rdbEncoder.saveDoubleValue(it->score);
        ...
        case DataType::LSIZE: {
            ...
            rocksdb::ReadOptions readOptions(false, true);
    ...
    *res = "none";
    ...
    if (s.ok()) {
        ...
        if(meta_val[POS_DEL] == KEY_ENABLED_MASK){
            ...
            s = ldb->Write(rocksdb::WriteOptions(), &(batch));
            if(!s.ok()){
                return STORAGE_ERR;
            }
...
int SSDBImpl::digest(std::string *val) {
    ...
    uint32_t crc = 0;
    while (mit->next()) {
        crc = rocksdb::crc32c::Extend(crc, mit->key().data(), (size_t) mit->key().size());
        crc = rocksdb::crc32c::Extend(crc, mit->val().data(), (size_t) mit->val().size());
    }
