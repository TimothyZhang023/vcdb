...
Binlog::Binlog(const std::string& binlog_path, const int file_size) :
  ...
  if (!slash::FileExists(manifest)) {
    ...
    if (!s.ok()) {
      log_fatal("Binlog: new %s %s",filename.c_str(), s.ToString().c_str());
    }


    s = slash::NewRWFile(manifest, &versionfile_);
    if (!s.ok()) {
      log_fatal("Binlog: new versionfile error %s", s.ToString().c_str());
    }
    ...
    if (s.ok()) {
      ...
      log_fatal("Binlog: open versionfile error");
    }

    profile = NewFileName(filename, pro_num_);
    log_info("Binlog: open profile %s", profile.c_str());
    s = slash::AppendWritableFile(profile, &queue_, version_->pro_offset_);
    if (!s.ok()) {
      log_fatal("Binlog: Open file %s error %s", profile.c_str(), s.ToString().c_str());
      }
