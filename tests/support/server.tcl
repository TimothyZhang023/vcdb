...
proc start_server_error {config_file error} {
    set err {}
    append err "Cant' start the vcdb server\n"
...
proc start_server {options {code undefined}} {
    ...
    # setup defaults
    set baseconfig "vcdb_default.conf"
    set overrides {}
    set tags {}
    ...
    set vcdbport [expr $::port]

    set data [exec cat "tests/assets/$baseconfig"]
    set config_file [tmpfile vcdb.conf]
    set fp [open $config_file w+]
    set data [regsub -all {{work_dir}} $data $workdir]
    set data [regsub -all {{vcdbport}} $data $vcdbport]
    ...
    set pid [exec vcdb $config_file > $stdout 2> $stderr &]
    ...
    if {$code ne "undefined"} {
        set serverisup [server_is_up $::host $vcdbport $retrynum]
    } else {
        set serverisup 1
    }
    ...
    if {$code ne "undefined"} {
        ...
        while 1 {
            ...
            if {[exec grep "vcdb server started." | wc -l < $stdout] > 0} {
                break
            }
            after 10
        }
